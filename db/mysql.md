...
分库分表,读写分离用mycat中间件
where查询中如果字段类型不匹配,会尝试类型转换,转换不了直接返回空数据,不报错

客户端 -> 连接器 -> 解析器(词法分析,语法分析) -> 预处理器 -> 优化器 -> 执行器
连接器: 身份校验; 传输语句
预处理器: 检查查询语句中的表或者字段是否存在; 将select * 中的*扩展为表中各个列
优化器: 转化为执行计划(explain)
```
...
select * from article where (title,content,uid) in (select title,content,uid from blog);   // 第一处括号不能省,子查询不能出现limit
...
4. InnoDB只包含一个*.ibd文件,MyISAM数据文件分离,包含*.frm(结构), *.MYD(数据文件),*.MYI(索引文件)
```
...
### buffer pool(innodb适用)

```
缓冲池是内存中的一个区域,缓存表和索引数据,缓冲池越大,InnoDB的行为就越像内存数据库
磁盘中按页为单位进行数据存储,缓冲池也是按页缓存数据,默认大小是16k
读写操作先从缓冲池中查看数据是否存在,如果存在直接读写,否则将数据所在页从磁盘读取到缓冲池,再进行读写操作
多线程并发访问同一个缓冲池时必然要加锁,导致很多线程要串行排队,可以通过innodb_buffer_pool_instances配置多个缓冲池提升并发
脏页是缓冲池中被修改过的页,与磁盘上的数据页不一致,脏页数据会在适当时候由引擎持久化到磁盘
```
...
```
事务提交之前发生崩溃,重启后会通过undo日志回滚事务,事务提交之后发生崩溃,重启后会通过redo日志恢复事务
undo日志: 
每条记录每修改一次都会记录修改前的数据并加上roll_pointer指针和trx_id事务id,指针指向数据前一个状态,形成版本链,可以用来回滚
redo日志: 
当有更新时先更新缓冲池对应数据,再将本次修改以redo日志形式写到log buffer,事务提交或使用内存接近innodb_log_buffer_size等情况时再写入磁盘
这里不能直接将更改数据写入磁盘而不用redo日志,应为写redo日志是顺序写磁盘,写数据是随机写磁盘,磁盘顺序写比随机写高效的多
redo日志是在崩溃恢复期间用于纠正不完整事务写入的数据,在初始化期间和接受连接之前,自动重播在意外关闭之前未完成更新数据文件的修改
SELECT
	FILE_ID,
	START_LSN,
	END_LSN,
	...
	CONSUMER_LEVEL 
FROM
	PERFORMANCE_SCHEMA.innodb_redo_log_files;  -- 查询redo日志信息
...
```
xa start 'trx';  # 全局唯一id,事务处于ACTIVE状态,start和end之间是事务操作
insert into tb_name values('she');
xa end 'trx';      # 事务处于IDLE状态
xa prepare 'trx';  # 事务处于PREPARED状态
xa commit 'trx';   # 结束事务
Xa rollback 'trx'; # 回滚事务

TM(Transaction Manager): 负责协调和管理事务,提供给AP应用程序编程接口和管理RM
RM(Resource Manager): 数据库服务端
XA事务: 基于两阶段提交(2PC)
准备阶段: TM给每个RM发送prepare消息,每个RM要么直接返回失败,要么在本地执行事务,写本地的redo和undo日志,但不提交
...
create [unique|fulltext] index index_name on t_name(... asc,.. desc,..) [invisible];  # 默认升序,不可见索引可以测试删除索引对查询性能的影响而无需进行破坏性更改
...
BTREE索引:
所有索引和数据都会出现在叶子结点,叶子结点形成有序双向链表,非叶子结点只存储索引;适宜范围查询;左前缀匹配;全值匹配
每个节点最大为一页,每个节点只需要一次I/O就可以完全载入,索引字段越小,占用磁盘空间越小,树高度越低
新增数据时会先查到叶子结点对应位置并插入,如果节点过大则依次向上分裂,非叶子结点分裂直接将键上移,叶子结点分裂将键拷贝一份新的再上移
字符串类型索引按字典序构建,联合索引按字段顺序构建
...
InnoDB一定会建立聚集索引,把实际数据行和相关的键值保存在一起
  1. 有主键时,根据主键自动创建聚集索引
  2. 没有主键时,会用第一个非空唯一索引列作为此表的聚集索引(跟主键唯一区别是可以有多个非空唯一索引)
...
覆盖索引(covering index)
一个查询语句只用从索引中就能够取得,避免了查到索引后再返回表操作,减少I/O提高效率,称之为覆盖索引
...
延迟索引关联
应为要查询的Myisam数据分散存储,涉及更多IO操作,所以优化前分页查询Myisam比Innodb更慢
分页查询需要先查询(offset+N)条数据,再丢掉前offset条数据,利用索引覆盖技术,先把主键查出来,避免大量回表查询,然后再根据查出来的主键进行join一次性取数据
