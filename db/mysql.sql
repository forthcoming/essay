...
? show    //显示show的语法
\s        //查看当前用户的信息
从服务器执行查询操作,降低主服务器访问压(实时性要求高的数据仍需要从主数据查询)
...
3. InnoDB是聚集索引,必须要有主键,通过主键索引效率很高,但辅助索引需要两次查询,先查询到主键,然后再通过主键查询到数据。因此主键不应该过大,因为主键太大,其他索引也都会很大;Myisam是非聚集索引,数据文件是分离的,索引保存的是数据文件的指针。主键索引和辅助索引是独立的。
