...
优点: 对于范围查询的效率很高,因为其数据是按照大小排列,找到索引也就找到了数据,而MyISAM索引和数据没有放在一块,索引文件是缓存在key_buffer中,索引对应的是磁盘位置,不得不通过磁盘位置访问磁盘数据
...
非聚簇索引叶结点包含索引字段值及指向数据页数据行的逻辑指针,并不是数据本身
MyISAM二级索引叶子节点存放的还是列值与行号的组合,叶子节点中保存的是数据的物理地址,主键索引和二级索引在存储结构上没有任何区别
MYISAM引擎的索引文件.MYI和数据文件.MYD相互独立
