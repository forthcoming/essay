...
class Worker(QThread):
    # 问题: 执行耗时长的任务时主界面假死(未响应)
    # 分析：在GUI程序中主线程也叫GUI线程,因为它是唯一被允许执行GUI相关操作的线程,对于一些耗时的操作,如果放在主线程中,就会出现界面无法响应的问题
    # 方案: 耗时部分采用QThread
    task_signal = Signal(int)  # 参数int就代表这个信号可以传一个整数

    def run(self):  # 线程将在run函数返回后退出
        # Qt关键特性之一是它使用信号和槽在对象之间进行通信
        # 如果一个信号连接到一个槽,那么当信号发出时该槽被调用
        # 一个信号可以连接到多个槽, 一个信号也可以连接到另一个信号;一个槽可以连接多个信号
        # 信号参数可以是任何Python类型
        # 连接可以同步或异步,可以跨线程进行
        for i in range(2):
            time.sleep(1)
