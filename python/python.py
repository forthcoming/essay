...
code
reached
from each instruction is therefore
atomic
from the point
...
This is especially
true
for the mass updates to dictionaries and lists.When in doubt, use a mutex!

#########################################################################################################################################

pickle
除个别外(如不能序列化lambda表达式), pickle.dumps可以序列化任何数据类型成b字符串, 并保留原有的数据(比如生成好的树,
                                                                                                  图结构)
...
class level by creating descriptors (Implementing Descriptors) for each variable name.
...
set_trace()
在该点打断(
from ipdb import set_trace, 或者直接在该处使用breakpoint
...
when
a
breakpoint is encountered.执行到下个断点处
l(ist)[first[, last]]   List
source
code
for the current file.Without arguments, list 11 lines around the current line or continue the previous listing.
j(ump)
程序跳到指定行
p
打印某个变量
...
expression.
n(ext)
让程序运行下一行, 如果当前语句有一个函数调用, 用n是不会进入被调用的函数体中
s(tep)
跟n相似, 但是如果当前有一个函数调用, 那么s会进入被调用的函数体中
q
退出调试
r(eturn)
继续执行, 直到函数体返回(Continue
execution
until
the
current
...
h(elp)
打印当前版本Pdb可用的命令, 如果要查询某个命令, 可以输入h[command], 例如: "h l"
查看list命令
pinfo2
Provide
extra
