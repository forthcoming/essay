...
class finalize:
    """
    finalize(obj, func, *args, **kwargs) returns a callable finalizer object which will be called when obj is garbage collected.
    The first time the finalizer is called it evaluates func(*arg, **kwargs) and returns the result. After this the finalizer is dead, and calling it just returns None.
    ...
    def peek(self):  # If alive then return (obj, func, args, kwargs); otherwise return None
        info = self._registry.get(self)
        obj = info and info.weakref()
        if obj is not None:
            return (obj, info.func, info.args, info.kwargs or {})
    ...
    @classmethod
    def _exitfunc(cls):
        # At shutdown invoke finalizers for which atexit is true. This is called once all other non-daemonic threads have been joined.
        reenable_gc = False
        try:
            if cls._registry:
                if gc.isenabled():
