...
---
##### 相互独立事件  
事件A/B是否发生对事件B/A发生的概率没有影响,则{a,B} {A,b} {a,b}都是相互独立事件  
P(AB)=P(A)P(B)  
P(B|A)=P(B)  
...
P(B)=P(B|A1)*P(A1)+P(B|A2)*P(A2)+P(B|A3)*P(A3)=(1/12)+(1/12)+(1/18)=2/9
P(A1|B)=3/8
```
...
---
##### Gamma函数
![Image](https://github.com/forthcoming/essay/blob/master/ml/source/gamma%20function.jpg)  
思考:   
编程实现快速计算n!   
Г(α+1)=αГ(α)可以通过分布积分证明   
...
详细的说是前k-1次皆失败,第k次成功的概率,记作X ~ G (p)  
E(X)=1/p
...
假定不存在季节因素,可以近似认为这个问题满足以下三个条件:  
- 顾客购买水果罐头是小概率事件;
- 购买水果罐头的顾客是独立的,不会互相影响;
- 顾客购买水果罐头的概率是稳定的.

在统计学上只要某类事件满足上面三个条件,它就服从泊松分布,它是单位时间内独立事件发生次数的概率分布     
泊(po)松分布的参数λ是单位时间(或单位面积)内随机事件的平均发生率  
...
---
##### 指数分布(连续型随机变量分布)
指数分布是泊松过程的事件间隔的分布,可以根据泊松分布推导得出    
...
正态分布有两个参数,即期望μ和标准差σ; 当μ=0,σ=1时称为标准正态分布  
正态分布关于x=μ对称,在x=μ处达到极大值,σ越小,曲线在x=μ附近越陡峭(通过x=μ处的最大值来判断)  
正态分布X～N(μ,σ^2)标准化 => Z=(X-μ)/σ  可以证明Y=aX+b服从正太分布  
...
```python
一射手进行射击,击中目标的概率为p(0<p<1),射击到击中两次为止.设以X表示首次击中目标所进行的射击次数,以Y表示总共进行的射击次数.
试求(1)X和Y的联合分布率;(2)条件分布率.
1. 
P(X=m,Y=n)=p^2q^(n-2), n=2,3,...;m=1,2,...,n-1
2.
P(X=m)=Σ(n=m+1,∞)p^2q^(n-2)=pq^(m-1), m=1,2,...
P(Y=n)=Σ(m=1,n-1)p^2q^(n-2)=(n-1)p^2q^(n-2), n=2,3,...
P(X=m|Y=n)=P(X=m,Y=n)/P(Y=n)=1/(n-1), m=1,2,...,n-1
P(Y=n|X=m)=P(X=m,Y=n)/P(X=m)=pq^(n-m-1), n=m+1,m+2,...
```
...
f(x|y)   
=lim(ε->0)∫(y,y+ε)f(x,y)dy/∫(y,y+ε)f(y)dy      
=lim(ε->0)f(x,y+ε)/f(y+ε)  # 洛必达法则     
=f(x,y)/f(y)   
称f(x,y)/f(y)为在Y=y的条件下X的条件概率密度  
条件概率密度f(x|y)可能含0到2个变量,在定义域内对x积分结果是1     
边缘概率密度f(x)一定只含一个变量x或直接是常数,在定义域内对x积分结果是1      
...
在几何上f(x,y)表示空间的一个曲面,介于她和xoy平面的空间区域体积为1   
...
某公司计划开发一种新产品市场,并试图确定该产品的产量.他们估计出售一件产品可获利m元,而积压一件产品导致n元的损失,
他们预测销售量Y(件)服从指数分布,其概率密度为f(y)=λe^(-λy) ; y>0,λ>0.问若要获得利润的数学期望最大,应生产多少件产品
设生产x件,则获利Q(x)={mY-nx+nY,Y<x; mx,Y>=x}
E(Q)
=∫(0,x)(my-nx+ny)f(y)dy+∫(x,∞)mxf(y)dy
=(m+n)/λ-(m+n)/[λe^(-λx)]-nx
然后对x求导使导函数等于0,求出x=[ln(1+m/n)]/λ,可以使E(Q)取到极大值
```
...
方差: D(X)=E{[X-E(X)]^2}=E(X^2)-[E(X)]^2  => E(X^2)=D(X)+[E(X)]^2         
标准差: σ(X)=√D(X)         
设C是常数,则有D(C)=0,D(aX+b)=a^2D(X)        
...
不相关是指X,Y不存在线性关系,但他们还可能存在其他关系(如平方关系)   
...
---
##### n维正态分布
n维正态分布的边缘分布都是一维正态分布,且不依赖于参数ρ,这也表明不能由边缘分布确定联合分布     
n维正态分布中Xi不相关等价于Xi相互独立    
![Image](https://github.com/forthcoming/essay/blob/master/ml/source/n维正太随机变量概率密度.png)          
...
正态分布是二项分布的极限分布,因此当n充分大时可以用正态分布近似计算    
