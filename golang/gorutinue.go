...
func testWaitGroup() {
	...
	go func(ch1 chan<- int, wg *sync.WaitGroup) { // 可以定义单向通道的channel,双向通道可以作为单向通道的入参
		...
		close(ch1) // 用完关闭,防止下面协程死锁,通道关闭后可以取到默认值,且第二个参数是false,无法向关闭的通道中发送数据
	}(ch1, &wg)
...
func testSelect() {
	ch := make(chan int)
	stop := make(chan bool)                           // 不带缓存的channel
	...
	for { // 死循环
		select { // 同时处理多个channel
		//case ch <- 1: // 如果成功向ch写入数据,则执行对应case处理语句
		case c := <-ch:
			fmt.Println("Receive", c)
		case _ = <-stop:
			goto ENDLOOP // 此处不能用break,应为break仅仅退出select,也可以用return
