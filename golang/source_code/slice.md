...
func nextslicecap(newLen, oldCap int) int {
	...
	for {
		// 从小切片增长2倍过渡,对于大切片增长1.25倍,这个公式在两者之间提供平滑的过渡
		newcap += (newcap + 3*threshold) >> 2

		// 我们需要检查`newcap >= newLen`以及`newcap`是否溢出,newLen保证大于零,因此当newcap溢出时,则`uint(newcap) > uint(newLen)`
		if uint(newcap) >= uint(newLen) { // 通过一个比较来检查两者
			break
		}
	}
